...
Swift source-language importer package for Frontier Lang semantic source documents.

Wraps the compiler SwiftSyntax native importer with package-level metadata, import helpers, and semantic sidecar generation for SwiftSyntax/SwiftParser-shaped syntax trees.
...
```js
import { importSwiftSource, createSwiftSemanticImportSidecar } from '@shapeshift-labs/frontier-lang-swift';
...
This package expects a caller-owned SwiftSyntax tree, parser module, or parser function. It records source-accurate syntax-tree metadata and semantic sidecars for merge review; it does not claim SourceKit symbols, type checking, macro expansion, active conditional-compilation branches, generated-source provenance, package resolution, or runtime semantics unless those are provided as evidence.

## API

- `createSwiftNativeImporterAdapter(options)`: create the package-level native importer adapter.
- `importSwiftSource(input, options)`: import source plus a native AST into a Frontier native import result.
- `createSwiftSemanticImportSidecar(input, options)`: import source and return a semantic import sidecar suitable for swarm merge evidence.
- `SwiftLanguagePackage`: package metadata for release-train and coordinator tooling.
